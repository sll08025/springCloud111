...
public class SpecParamDTO extends BaseDTO {
    ...
    @ApiModelProperty(value = "是否数字类型 1true 0false ",example = "0")
    @NotNull(message ="是否数字类型不可为空",groups = {MingruiOperation.Update.class,MingruiOperation.Add.class})
    private Boolean numeric;

    @ApiModelProperty(value = "数字类型参数的单位 非数字类型可以为空")
    private String unit;

    @ApiModelProperty(value = "是否sku通用属性 1true 0false",example = "0")
    @NotNull(message = "是否是sku通用属性不能为空",groups = {MingruiOperation.Add.class,MingruiOperation.Update.class})
    private Boolean generic;

    @ApiModelProperty(value = "是否搜索过滤 1true 0 false",example = "0")
